...
def train():
    ...
    if conf["continue_training"] and os.path.exists("./model/generator.ckpt"):
        mindspore.load_checkpoint("./model/generator.ckpt", generator)
        mindspore.load_checkpoint(
            "./model/surface_discriminator.ckpt", surface_discriminator)
        mindspore.load_checkpoint(
            "./model/texture_discriminator.ckpt", texture_discriminator)
    ...
    def discriminator_forward(real: Tensor, fake: Tensor) -> Tuple[Tensor]:
        ...
        surface_loss = lsgan_loss_d(surface_discriminator(
            real_guided), surface_discriminator(fake_guided))
        texture_loss = lsgan_loss_d(texture_discriminator(
            real_gray), texture_discriminator(fake_gray))

        loss = surface_loss+texture_loss
    ...
    total = min(len(real_train_loader), len(real_test_loader))
        ...
        for i, (real, cartoon) in enumerate(zip(real_train.create_tuple_iterator(), cartoon_train.create_tuple_iterator())):
            ...
            if i % 100 == 0 or i == total-1:
                print("[%d/%d][%d/%d] Loss_D:%7.4f Loss_G:%7.4f" % (
                    epoch+1, conf["epoch"], i+1, total, d_loss.asnumpy(), g_loss.asnumpy()))

            D_losses.append(d_loss.asnumpy())
            G_losses.append(g_loss.asnumpy())

        mindspore.save_checkpoint(generator, "./model/generator.ckpt")
        mindspore.load_checkpoint(surface_discriminator,
                                  "./model/surface_discriminator.ckpt")
        mindspore.load_checkpoint(texture_discriminator,
