...
def train(generator:Generator, surface_disc: Discriminator, texture_disc: Discriminator,device:str="CPU"):
    ...
    if conf["continue_training"] and "generator.pth" in os.listdir("model"):
        gen_checkpoint=torch.load("model/model.pth")
    ...
    generator_optimizer=torch.optim.Adam(generator.parameters(),lr=conf["lr"])
    surface_disc_optimizer=torch.optim.Adam(surface_disc.parameters(),lr=conf["lr"])
    texture_disc_optimizer=torch.optim.Adam(texture_disc.parameters(),lr=conf["lr"])
